...
def preprocess(afbeelding): #de afbeelding is in het eindpunt al ingelezen met cv2, dat hoeft niet meer
    ...
    kernel = np.ones((3, 2), np.uint8) #maakt een 3x2 matrix (numpy array) aan gevuld met alleen maar integer 1en. deze wordt gebruikt als kernel (een soort filter) om de afbeelding in de volgende regel over te gaan.
